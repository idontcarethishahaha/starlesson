...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    @Override
    public PageDTO<UserPageVO> queryUserPage(UserPageQuery pageQuery) {
        // 1. 分页查询，不筛选用户类型（查询所有用户）
        Page<UserDetail> page = detailService.queryUserDetailByPage(pageQuery, null);
        // 2. 处理 VO 转换
        return PageDTO.of(page, u -> {
            UserPageVO v = BeanUtils.toBean(u, UserPageVO.class);
            // UserDetail 的 type 是 UserType 枚举，转换为 Integer 值
            if (u.getType() != null) {
                v.setType(u.getType().getValue());
            }
            return v;
        });
    }
