...
// 新增会话 - tj-chat: POST /session，body: { name, tag }
// 后端自动创建新会话，返回 UserSession 对象（含 id 和 sessionId）
const createSession = async () => {
    try {
        const res = await createUserSession({ name: formData.value.name, tag: formData.value.tag });
        if (res && res.id) {
            emits('createSession', res.sessionId); // 通知父组件使用新的 sessionId
...
// 修改会话 - tj-chat: PUT /session/{id}，params: { name, tag }
const updateSession = async (id) => {
    try {
        await updateUserSession(id, { name: formData.value.name, tag: formData.value.tag });
        emits('updateSession');
...
// 删除会话 - tj-chat: DELETE /session/{id}
const handleDeleteConfirm = async () => {
    try {
        await deleteUserSession(deletingSessionId.value);
        emits('deleteSession');
