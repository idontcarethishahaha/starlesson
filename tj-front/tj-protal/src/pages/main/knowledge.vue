<template>
    <div class="aiChatWrapper" @click="handleGlobalClick">
        ...
        <el-dialog v-model="isAddModalVisible" title="新增文件" @close="handleAddModalClose">
            <el-form :model="addFormData" :rules="addFormRules" ref="addFormRef" label-width="80px">
                ...
                <div style="text-align: left;margin-left: 80px;" class="el-upload__tip">请上传2MB以内的Markdown文件</div>
                <el-form-item prop="fileName">
                    <div v-if="addFormData.fileName">{{ addFormData.fileName }}</div>
                </el-form-item>
                <el-form-item label="切割等级" prop="level">
                    <el-input-number v-model="addFormData.level" type="number" max="5" min="1"></el-input-number>
                </el-form-item>
            </el-form>
            <template #footer>
        ...
        <el-dialog v-model="isEditModalVisible" title="修改文件" @close="handleEditModalClose">
            <el-form :model="editFormData" :rules="editFormRules" ref="editFormRef" label-width="80px">
                ...
                <el-form-item label="切割等级" prop="level">
                    <el-input-number v-model="editFormData.level" type="number" max="5" min="1"></el-input-number>
                </el-form-item>
            </el-form>
            <template #footer>
                <div class="dialog-footer">
        ...
        <!-- 删除确认对话框 -->
        <el-dialog v-model="deleteConfirmVisible" title="删除确认" @close="handleDeleteCancel">
            <p>确定要删除该文件吗？</p>
            <template #footer>
        ...
        <!-- 查看文件内容对话框 -->
        <el-dialog v-model="viewFileVisible" title="文件内容" @close="handleViewFileClose">
...
const addFormRules = ref({ file: [{ required: true, message: '文件为必填项', trigger: 'change' }] });
...
const editFormRules = ref({ content: [{ required: true, message: '文件内容为必填项', trigger: 'change' }] });
...
const fetchFileList = async (page = 1) => {
    try {
        ...
        // tj-chat PageDTO: { data: { records: [...] } } -> axios 包装后: { records: [...] }
        ...
        totalFiles.value = response.total || list.length;
    } catch (error) {
...
// 提交修改文件 - tj-chat: PUT /file/update
...
const updateFile = async (id) => {
    try {
        await updateMarkdown({ id, content: editFormData.value.content, level: editFormData.value.level });
        ElMessage.success('修改成功');
        isEditModalVisible.value = false;
        const fileData = await getMarkdown(id);
        viewFileContentData.value = fileData;
        currentPage.value = 1;
        fetchFileList();
    } catch (error) {
...
// 确认删除 - tj-chat: DELETE /file/{id}
const handleDeleteConfirm = async () => {
    try {
        await deleteMarkdown(deletingFileId.value);
...
// 提问 - tj-chat: GET /file/chat?message
const askQuestion = async () => {
    ...
    try {
        ...
        // tj-chat 返回 List<MarkdownChunkVO>: [{ title, content, score }]
        if (res && Array.isArray(res) && res.length > 0) {
            const markdownChunks = res.map(chunk => ({
                title: chunk.title,
                content: chunk.content,
                contentHtml: md.render(chunk.content),
                score: chunk.score
            }));
            chatMessagesList.value.push({ type: 'markdownChunk', chunks: markdownChunks });
        } else {
            ElMessage.warning('知识库中未找到相关内容');
            chatMessagesList.value.push({ type: 'assistant', processedContent: '<p>未找到相关内容，请尝试其他关键词或上传更多文档。</p>' });
        }
    } catch (error) {
